...
<body>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-sm-flex justify-content-between align-items-center">
									<h2>Data OPL</h2>
									<button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-info col-md-2"><span class="glyphicon glyphicon-plus"></span>Tambah Data OPL</button>
                                </div>
                                <ol class="breadcrumb mb-4">
                                    <ul class="breadcrumbs pull-left">
                                        <li><a href="index.php">Home</a></li>
                                        <li><span>Data OPL</span></li>
                                    </ul>
                                </ol>
                                <div class="market-status-table mt-4">
                                    <div class="table-responsive">
										 <table id="dataTable3" class="table table-hover"><thead class="thead-dark">
                                            ...
                                            <?php
                                            $data = query("SELECT * FROM dataopl 
                                            INNER JOIN tbl_department ON dataopl.department_id = tbl_department.department_id
                                            INNER JOIN tbl_lokasi ON dataopl.lokasi_id = tbl_lokasi.lokasi_id  
                                            WHERE idx order by lokasi_nama, department_nama, nama_mesin");
                                            $no = 0;
                                            foreach ($data as $d) :
                                                    $no = $no + 1;
                                                    $idx = $d['idx'];
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $d['nama_mesin']; ?></td>
                                                <td><?= $d['lokasi_nama']; ?></td>
                                                <td><?= $d['department_nama']; ?></td>
                                                <td><?= $d['title']; ?></td>
                                                <td><?= strtoupper($d['ekstensi']) ?></td>
                                                <td><?= number_format($d['size']/(1024*1024), 2) ?>MB</td>
                                                <td><a href="DownloadFile.php?url=<?= $d['berkas']; ?>">Download</a> | <a href="cek.php?url=<?= $d['berkas']; ?>">Cek File</a></td>
                                                <td><a href="javascript:del(<?= $d['idx'];?>)" class="btn btn-danger" title="Hapus"><i class="ti-trash"></i></a> <button data-toggle="modal" data-target="#edit<?=$idx;?>" class="btn btn-warning" title="Edit"><i class="ti-pencil-alt"></i></button></td>
                                            </tr>

                                            <!-- The Modal -->
                                            <div class="modal fade" id="edit<?=$idx;?>">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                    <form method="post" enctype="multipart/form-data" action="update.php">
                                                        <!-- Modal Header -->
                                                        <div class="modal-header">
                                                        <h4 class="modal-title">Edit File OPL <?php echo $d['nama_mesin']?></h4>
                                                        ...
                                                        <input type="text" id="nama_mesin" name="nama_mesin" class="form-control" value="<?php echo $d['nama_mesin'] ?>" >

                                                        <label>Lokasi</label>
                                                        <label class="form-control"><?php echo $d['lokasi_nama']; ?></label>
                                                        <input type="hidden" name="lokasi_id" value="<?php echo $d['lokasi_id']; ?>">

                                                        <label>Area</label>
                                                        <label class="form-control"><?php echo $d['department_nama']; ?> </label>
    ...
    <div id="myModal" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
		    	<div class="modal-header">
					<h4 class="modal-title">Masukkan Data OPL</h4>
			    </div>
                <div class="modal-body">
                    <form action="ScriptFileUpload.php" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label>Nama Mesin</label>
                            <input type="text" name="nama_mesin" value="" class="form-control" placeholder="Nama Mesin" required>
                        </div>
                        <div class="form-group">
                            <label>Lokasi</label>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="department_id" id="department_nama" required></select>
                        </div>
                        <div class="form-group">
                            <label>Upload File</label>
                            <input type="file" name="berkas" value="" class="form-control" accept="application/pdf" required>
                        </div>
                        <div class="modal-footer">
